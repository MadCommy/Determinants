...
\textbf{Cases when the LU factorization exists.}
  ...
  \begin{itemize}
      \item A $LUP$ factorization will always exist for a square matrix $A$.
      \item If a square matrix A is invertible and all its leading principal minors are nonzero, then a LU factorization will exist.
...
\begin{itemize}
     \item If $A$ is a singular matrix (i.e. det$(A) = 0$), with $rank(A)=k$, we can’t know for sure if the $LU$ factorization will exist.  
     
         \begin{itemize}
             \item If the first $k$ leading principal minors are nonzero, the LU factorization exists.
             \item For an $nxn$ matrix $A$ the $LU$ factorization exists if $$rank(A_{1,1}) + n \geq rank\begin{pmatrix}A_{1,1} & A_{1,2}\end{pmatrix} + rank\begin{pmatrix}A_{1,1} \\ A_{2,1} \end{pmatrix}$$
         \end{itemize}
\end{itemize}

\end{frame}

\begin{frame}{Does LU decompostion always work (cont.)}

Simple example of when LU decompostion fails 
\bigbreak
\[
\begin{pmatrix}
0 & 1\\ 1 & 0
...
\begin{frame}{How it helps us compute determinants (cont.)}
   ...
   \begin{align}
    \text{det}(A) &= \text{det}(P^T)\text{det}(L)\text{det}(U) \\
             &= \text{det}(P)\text{det}(L)\text{det}(U) \\ 
    \end{align} 
    We can make this step as we know from algebra that, $\text{det}(AB) = \text{det}(A)\text{det}(B)$ and also that $\text{det}(A) = \text{det}(A^T)$.
...
$$ \text{det}(A) = (-1)^s \left 
(\prod_{i=1}^{n} l_{ii} \right ) \left (\prod_{i=1}^{n} u_{ii} \right ) $$
